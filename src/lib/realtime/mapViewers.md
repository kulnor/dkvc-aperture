...
The account ids (`ap_user.id`) that currently have `mapId` open in a live socket. `GET /api/map/[mapId]/viewers` expands these to the character ids those accounts own.
