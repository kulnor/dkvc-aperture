...
- **`signIn` callback (the login gate):** runs *before* `jwt`, so a denial issues no session/JWT and the `jwt` branch (`persistLogin`/`syncCharacterAuthz`) never runs — no `ap_character` row is created for a rejected sign-in. Reads the character id from the verified profile, then fetches corp/alliance via the public (token-less) `esiCall('getCharacter', …)`; on ESI failure it degrades to character-level checks (corp/alliance left `null`, logged as a warning, never fails open). Defers the decision to `isLoginAllowed` (`@/lib/auth/loginGate.ts`), which enforces `access_mode`, the owner/allowlist checks, and the unconfigured-instance bootstrap.
- **`jwt` callback:**
...
Notes:
- Corp/alliance ids are refreshed by `syncCharacterAuthz` on every sign-in.
