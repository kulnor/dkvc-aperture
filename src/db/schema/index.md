...
Re-exports `universe/{geography,items,dogma,statics,views,sovereignty}`, `ap/{enums,instance,access_grant,user,character}`, the map schema `ap/{map,map_system,map_connection,map_signature,map_event,event_kind}`, and `ap/{system_stats,job_run,map_character_tracking,map_tracking_seed,webhook,corporation,alliance,role,corporation_right,structure}`. Imported as `* as schema` by `src/db/client.ts` and globbed by `drizzle.config.ts`.
