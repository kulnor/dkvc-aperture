...
- `authz_level` — `authz_level` enum (`member | admin`), default `member`. Recomputed cache: `syncCharacterAuthz` writes `admin` iff an explicit `ap_access_grant` (`capability='admin'`) exists, else `member`. The EVE Director role does NOT raise it — corp/alliance map authority is the separate `is_director` bit.
- `is_director` — `boolean`, default `false`. The EVE corporation "Director" role, refreshed from ESI on every `syncCharacterAuthz` pass. Carries corp/alliance map-management authority in the derived-authority model (`canManageMap` / `canCreateMap` in `src/lib/auth/rights.ts`); distinct from `authz_level` (the instance-operator tier).
