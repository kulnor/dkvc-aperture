...
**Goal:** A clean Next.js 15 + React 19 + TS 5 repo with Drizzle, Auth.js v5, shadcn/ui, sonner, Tiptap, xyflow, TanStack Table installed; `docker compose` brings up Postgres 16 with `pgcrypto` and `pg_partman`; `pnpm dev` boots an empty App Router page; `pnpm typecheck`, `pnpm lint`, `pnpm test` (Vitest) all pass; CI runs the same.
