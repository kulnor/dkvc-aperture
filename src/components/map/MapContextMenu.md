...
- **pane** — `Add system`; plus a destructive `Delete disconnected` (Unlink icon) shown only when a Home is set **and** `computeDisconnected` finds ≥1 system cut off from it (otherwise the action is a no-op, so it's hidden). Greyed via `DisabledHintItem` when any disconnected system is locked.
