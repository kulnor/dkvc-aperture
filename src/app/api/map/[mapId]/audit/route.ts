...
import { auditActorSummary, queryAuditEvents, type AuditQueryParams } from '@/lib/map/audit';
 ...
 * Keyset-paginated commit feed for the manager audit console. Read-only.
 *
 * Access: managers/admins only. A plain member with view access must NOT read
 * the audit feed, so this layers `isManagerOrAdmin` on top of `requireMapView`
 * (which scopes the map and returns 404 to avoid leaking existence).
 *
...
export async function GET(_request: NextRequest, { params }: { params: Promise<{ mapId: string }> }) {
  ...
  return Response.json({ ok: true, data: { ...page, actorSummary } });
}
