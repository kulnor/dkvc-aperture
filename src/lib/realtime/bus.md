...
- On notification, parses the payload as JSON (non-JSON → `{}`), derives `mapId` from the channel name, and emits one of two envelopes:
  ...
  - **`{ task: 'systemNotification', load }`** — when the pg_notify payload has a top-level `task: 'systemNotification'`. Two producers: the Stage 17.8 zKB feed (`kind: 'killmail'`) and the user-initiated ping (`kind: 'ping'`, `src/lib/map/ping.ts`). The bus validates the `load` against `systemNotificationLoadSchema` (a `kind`-discriminated union) and drops malformed envelopes silently.
