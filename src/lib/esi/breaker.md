...
**Purpose:** Per-endpoint (keyed by swagger `operationId`) circuit breaker so the ESI client stops hammering a failing endpoint. In-process `Map`, no shared store (no Redis).
