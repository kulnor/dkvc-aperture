...
- Each row carries the Tailwind `group` class; the action button is a primary (`variant="default"`) `Go →` button that is `opacity-0` by default and only fades in on row hover (`group-hover:opacity-100`) or keyboard focus (`focus-visible:opacity-100`). The whole row is intentionally **not** clickable — navigation closes the dialog and reframes the map, so it must be a deliberate button press, leaving text selection / stray clicks safe.
- Clicking the `Go →` button on a result row calls `onNavigate(system.id, sig.id)`, which closes the dialog, selects the system, centers the canvas, and starts a 3-second row flash in `SignatureModule`.
